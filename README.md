...
**LeetCode 存的是提交记录，这里存的是解题过程。**

一串时间戳没法复习——当时为什么选这个思路、复杂度怎么算的、那几个测试用例各自在防什么，都不在里面。所以这里一道题一个目录：代码、思路、复杂度、用例放在一起，三门语言并排写，根 README 自动汇总。

其余功夫都花在让记录别太费劲上，因为一费劲就不会坚持：`make new ID=1` 一条命令开题，题解本体和 LeetCode 提交框逐字一致，复制进出不改一个字。

想要同样一个仓库，不用 fork 我的题解——从 `init` 标签起步，那里是干净的骨架和工具链。
...
|Completion Rate|0.2%|0.0%|0.0%|0.0%|
