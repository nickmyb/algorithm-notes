...
我的算法题解仓库，支持 Go / Python / Java 多语言题解。**clone 下来三条命令就能开始刷题。**

## 初心

刷题本身已经够费脑子了，不该再把精力花在环境上。

自己折腾过一轮之后发现，真正开始写第一行代码之前，往往要先趟一遍这些：建目录、想名字、复制题面、写测试骨架、配 IDE、调通运行配置——每样都不难，但加起来足够磨掉刚燃起来的那点劲头。等终于跑通了，最初想解的那道题早就凉了。

所以这个仓库想做的很简单：**把刷题之外的事情全部前置掉，让你 clone 完就能直接开始。**

```sh
git clone git@github.com:nickmyb/algorithm-notes.git && cd algorithm-notes
git switch -c my-solutions init
make init
make new ID=1        # 目录建好、题面填好，打开就能写
```

具体到细节：

- **`make new ID=1` 一条命令搞定开题**——自动查标题建目录、抓英文题面和官方中文翻译写进 README、按你要的语言复制骨架。不用手敲目录名，不用来回复制粘贴题目。
- **写完直接 `make test ID=1`**，三门语言的测试链路已经接好，共享的 `TreeNode` / `ListNode` 也都备好了，不用每道树的题重抄一遍节点定义。
- **题解本体和 LeetCode 提交框逐字一致**，来回复制不用改一个字。
- **输出里没有噪音**。日常测试不跑骨架目录，Python 的工具链测试只给一行结论，每门语言有清楚的开始和结论——你只会看到自己那道题的用例名和结果。
- **用不用 IDE 都行**。编辑器 + `make test` 足够；想在 JetBrains 里写，`make ide` 一条命令建好三个独立项目，命令结束时会把该点哪里、填什么原样打出来。
