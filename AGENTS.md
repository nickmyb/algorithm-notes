...
### 10. 题解 README 由作者本人写，助手不要代笔

`ctl new` 会自动填「题目」（英文原文 + 折叠的官方中文翻译），**其余小节一律不碰**。

这条对 AI 助手是硬约束：**不要往 `leetcode/<题目>/README.md` 里自动补内容**，包括看起来"明显有用"的推导、对比表格、实测数据。这个仓库存在的理由就是记下作者自己怎么想通的（见 README 的「初心」），助手代笔等于把这件事抹掉——作者只保留他自己觉得有用的那部分，取舍权在他。

踩过一次：作者让"帮我提交 0104"，助手顺手往题解 README 里塞了一段复杂度推导、一张实测数据表和一句补充说明，全是作者没要求的。

边界：

- **可以**：作者明确说"帮我填复杂度 / 写进 README"时照做；对话里把分析讲清楚，由作者自己决定抄哪些进去。
- **不可以**：以"顺手"、"补全"、"这段有用"为由自行增补。
- **渲染 bug 单独说**：发现代码块没闭合、表格错行这类问题，**先报告，不要直接改**——改法往往牵动文字本身，那还是代笔。

### 11. 题解 README 的「题目大意」故意留空
...
**为什么不顺手解析题面把用例也生成出来**：`ctl new` 已经把题目描述抓下来了，看上去只差解析 Example 块。但 LeetCode 的示例结构自己就不统一，实测（2026-09）：

| 题 | 英文版 | 中文版 |
|:---|:---|:---|
| 1 | `<pre>` | `<pre>` |
| 94 | `<div class="example-block">` | **`<pre>`** |
| 3304 | `<div class="example-block">` | `<div class="example-block">` |

官方正在往 `example-block` 迁移，但迁了一半，而且**同一道题的中英文版本可能用不同结构**。再叠加 `Input:` / `输入：` 两种前缀、有的题输入是多个具名参数、有的返回值需要特殊比较（浮点、任意顺序、原地修改），解析出来的用例只会「大部分对」。

而**测试用例错了比没有更糟**——它会让错误的题解显示为绿色。所以这一步保持手工转录：人照着 README 里的 Example 抄一遍，错了立刻能发现。
...
4. 发布内容检查、测试通过后，才按用户确认的发布范围更新 `init`（判据见下节）；开发提交不得自动移动标签。`main` 也只有在用户明确要求时才接收工具链更新。不要为了「同步」强行把所有分支指向同一提交。
...
- **收录一道题用一个提交**：题解、测试和 `make readme` 刷新后的根 README 放在一起。分开提交会留下一个仓库自相矛盾的中间状态——题解文件在了，而根 README 的统计和题目列表里还没有它；回退这道题也要操作两次。一次收录多道题时可以每题一个提交，最后单独提交一次 README 刷新。

  顺序是**先提交题解，再 `make readme`**。统计扫的是磁盘不是 git（见约束 9），目录放在那里就会被算进去，反过来做会让 README 列出还没提交的题。

- **带助手尾注的题解提交，要在正文说明助手到底做了什么。** 题解本体是作者写的，尾注光秃秃挂在那里会被读成"这题是 AI 写的"。例如第 94 题署了 Claude，是因为 README 的复杂度分析由它推导、测试用例的取舍由它对拍验证，而 `Solution.*` 三个文件是作者本人写的——这句话要写进提交正文，不能只留一行尾注让人猜。
