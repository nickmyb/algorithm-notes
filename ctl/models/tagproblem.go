...
// 这个文件是 GraphQL 拉取「算法专题」题目列表用的模型，只服务于上游 LeetCode-Go 的
// Hugo 站点第二章渲染（ctl/render_website.go）。本仓库没有引入 website/，
// 所以整个文件用 //go:build ignore 停用，不参与编译。
//
// 要恢复：补回 website/ 目录，然后删掉本文件第一行的 //go:build ignore，
// 同时放开 ctl/render_website.go。
...
func GenerateTagMdRows(solutionIds []int, metaMap map[int]TagList, mdrows []Mdrow, internal bool) []TagList {
	...
	for _, row := range mdrows {
		if util.BinarySearch(solutionIds, int(row.FrontendQuestionID)) != -1 {
			...
			s7 := StandardizedTitle(row.QuestionTitle)
			if internal {
