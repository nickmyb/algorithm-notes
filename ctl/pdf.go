...
// 这个文件负责把整本《LeetCode Cookbook》合并成一个大 Markdown 以便导出 PDF，
// 依赖上游 LeetCode-Go 的 website/content/ 目录结构。本仓库没有引入 website/，
// 所以整个文件用 //go:build ignore 停用，不参与编译。
//
// 要恢复：
//  1. 补回 website/ 目录和 ctl/template/ 下的章节模板
//  2. 删掉本文件第一行的 //go:build ignore
//  3. 在 ctl/command.go 的 init() 里放开 newPDFCommand()
//
// 注意文件里的 books.halfrost.com 链接和 PDF 前言都是原作者的内容，恢复时要改成自己的。
